    ...
    <SafeAreaView style={[{ width }, tw` pt-10 pb-8`]}>
      ...
      <ScrollView style={tw`w-full mt-4 px-3 pb-8`}>
          {hotelSlides.map((slide, index) => (
            <>
              <Bookmark item={slide} key={index} open={()=> openModal(slide)}/>
            </>
          ))}
      </ScrollView>
      <Modal isVisible={showModal} style={tw`flex-1 items-center justify-center`}>
      <View
  style={[
    { width: width * 0.96 },
...
>
  <Text style={[{fontFamily: "Poppins-Bold"}, tw`text-xl mb-4`]}>Remove from Bookmark?</Text>
  <Text style={[{width: width * 0.96}, tw`border border-neutral-200 h-0`]}></Text>
